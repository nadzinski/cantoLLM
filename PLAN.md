...
**Status (2026-08-16):** OpenAI Chat Completions surface landed 2026-04-19
...
`--api {anthropic,openai}` (default anthropic). Two of the three leftovers
closed in Phase 3.5 (2026-08-16): stream error-event parity and the
`X-Request-ID` middleware. Still open: raw-tokens NDJSON endpoint (a debug
tool, not production hygiene; unscheduled).
...
at parity and at undercommitted capacity. Chunk 6 closed the same day:
the paged shape vocabulary drops the kv axis (one entry per
(batch, width) family; measured at the standard geometry, sweep shapes
315 to 20 and decode shapes 80 to 5, prediction 2's structural half),
warm-up fills the scheduler's own persistent buffers with scratch-parked
all-filler steps and builds every family's FlexAttention BlockMask
behind Ready, masks are cached once per family over those buffers (a
persistent start_pos buffer keeps a reused mask reading current step
values), and the whole batched forward now traces under torch.compile on
the paged path, one artifact per family with the paged write-map length
the only symbolic dim. Both chunk-6 gates green on CPU: kv-length
changes recompile nothing, and served traffic constructs zero masks per
step. Chunk 7 ran the first 5090 round the same day: flex wired
end-to-end behind `--attention flex` (implies the paged stack), and
three box runs closed it: a 4x multi-row decode cliff traced to
automatic dynamic disqualifying the flex-decoding kernel
(`static_batch`), fixed by per-family static batch dims; the warm
sweep trimmed to one forward per family after the fix doubled the
Ready bill; final standing: short_chat and code inside the -10% gate,
multi_turn and long_context outside it (worst -26.6%), attributed by
...
- **Reference-parity tests** against HF `transformers` for every new backend (padded,
  paged, quant, TP). `tests/test_hf_parity.py` is the seed (per-token logprob +
  greedy-argmax parity on the real 0.6B weights, fp32/CPU; reference regenerated by
  `tests/reference/generate_reference.py`) — extend it, don't let it bit-rot.
